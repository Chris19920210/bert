...
def file_based_input_fn_builder(input_files, is_training, batch_size):
    ...
    def _decode_record(record, name_to_features):
        ...
        return example["item_ids"].values, example["rates"].values, example["also_view_ids"].values, \
            example["user_id"][0]
    ...
    def input_fn():
        ...
        d = d.map(lambda item_ids, rates, also_view_ids, user_id: (
            item_ids[:FLAGS.max_items_length],
            rates[:FLAGS.max_items_length],
            also_view_ids[:FLAGS.max_also_view_length],
            user_id
        ))
        ...
        features = batched_dataset.map(lambda item_ids, rates, also_view_ids, items_mask, also_view_mask, user_id:
        {
            "item_ids": item_ids,
            "rates": rates,
